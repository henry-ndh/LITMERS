...
export const useGetProfile = () => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await BaseRequest.Get(`/api/auth/get-profile-by-user`);
      return res?.data || res;
    }
  });
};
...
export const useUpdateProfile = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (model: { name?: string; avatar?: string }) => {
      return await BaseRequest.Put(`/api/auth/update-profile`, model);
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ['profile', 'current']
